                ...
                with gevent.Timeout(TASK_TIMEOUT):
                    ...
                    try:
                        ...
                        if post_resp.status_code != 202:
                            ...
                            logger.warning(
                                f"[user={user_id}/{subscription_tier}] upload failed — {issue} "
                            ...
                            try:
                                ...
                                if not gid:
                                    ...
                                    logger.info(
                                        f"[user={user_id}/{subscription_tier}] upload OK — gid={gid}"
                                    )
                            except Exception as exc:
                                failed = upload_failed = True
                    ...
                    if not failed:
                        try:
                            ...
                            if gen_resp.status_code != 202:
                                ...
                                logger.info(
                                    f"[user={user_id}/{subscription_tier}] [gid={gid}] generate OK"
                                )
                        except gevent.Timeout:
                            raise
                        except Exception as exc:
